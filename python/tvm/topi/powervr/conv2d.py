...
"""conv2d schedule on Imagination PowerVR GPU"""
import logging
import tvm
from tvm import te
...
@autotvm.register_topi_compute("conv2d_nchw_spatial_pack.powervr")
...
@autotvm.register_topi_schedule("conv2d_nchw_spatial_pack.powervr")
...
@autotvm.register_topi_compute("conv2d_nhwc_spatial_pack.powervr")
...
@autotvm.register_topi_schedule("conv2d_nhwc_spatial_pack.powervr")
...
@autotvm.register_topi_compute("conv2d_nchw_winograd.powervr")
...
@autotvm.register_topi_schedule("conv2d_nchw_winograd.powervr")
...
@nn.conv2d_alter_layout.register(["powervr"])
def _alter_conv2d_layout(attrs, inputs, tinfos, out_type):
    ...
    if topi_tmpl == "conv2d_nchw_spatial_pack.powervr":
        ...
        new_workload = autotvm.task.args_to_workload(
            ...
            "conv2d_nchw_spatial_pack.powervr",
        )
        dispatch_ctx.update(target, new_workload, cfg)

        return relay.nn.conv2d(*inputs, **new_attrs)
    elif topi_tmpl == "conv2d_nchw_winograd.powervr":
        assert data_layout == "NCHW" and kernel_layout == "OIHW"
        N, CI, H, W = get_const_tuple(data.shape)
        CO, _, KH, KW = get_const_tuple(kernel.shape)
        tile_size = _pick_tile_size(data, kernel)
...
@conv2d_winograd_nhwc.register(["powervr"])
def conv2d_winograd_nhwc_powervr(
    data,
    weight,
    strides,
    padding,
    ...
    This is a clean version to be used by the auto-scheduler for powervr.
    """
