...
"""dense schedule on Imagination PowerVR GPU"""
from tvm import te
...
@autotvm.register_topi_compute("dense.powervr")
def dense(_, data, weight, bias=None, out_dtype=None):
    """Dense operator on powervr"""
    return nn.dense(data, weight, bias, out_dtype)


@autotvm.register_topi_schedule("dense.powervr")
def schedule_dense(cfg, outs):
    ...
    def _callback(op):
        if op.tag == "dense":
            ...
            # fallback support
            if cfg.is_fallback:
                ref_log = autotvm.tophub.load_reference_log("powervr", "v9h", "dense.powervr")
